...
describe('sync (Merkle tree)', () => {
  ...
  describe('computeChanges', () => {
    ...
    it('detects no changes when Merkle root matches', async () => {
      ...
      await persistMerkleState(files, first.fileHashMap, new Set(files), tmpDir);
    ...
    it('detects new file added', async () => {
      ...
      await persistMerkleState([fileA], first.fileHashMap, new Set([fileA]), tmpDir);
    ...
    it('detects deleted file', async () => {
      ...
      await persistMerkleState([fileA, fileB], first.fileHashMap, new Set([fileA, fileB]), tmpDir);
  ...
  describe('persistMerkleState', () => {
    it('only persists hashes for successful files', async () => {
      ...
      await persistMerkleState(files, result.fileHashMap, new Set([fileA]), tmpDir);
