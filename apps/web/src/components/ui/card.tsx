...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        'tui-corners group/card flex flex-col gap-4 overflow-visible bg-card py-4 text-xs/relaxed text-card-foreground has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:gap-2 data-[size=sm]:py-3 data-[size=sm]:has-data-[slot=card-footer]:pb-0',
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      ...
      className={cn(
        'flex items-center border-t border-border/30 p-4 group-data-[size=sm]/card:p-3',
        className,
      )}
      {...props}
    />
